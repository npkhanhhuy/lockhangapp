...
export default function JobMonitor({ navigation }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = cJOBS.onSnapshot((response) => {
      ...
      // Lọc danh sách dự án theo monitor
      const filteredJobs = arr.filter(
        (job) => job.monitor === userLogin.fullname
      );
  ...
  useEffect(() => {
    if (Array.isArray(jobList) && name && name.trim() !== "") {
      ...
      // Lọc jobData dựa trên tên tìm kiếm
      setJobData(
        filteredJobs.filter((job) => job.monitor === userLogin.fullname)
      );
    } else {
      setJobData(jobList.filter((job) => job.monitor === userLogin.fullname)); // Nếu không có điều kiện lọc, gán jobData bằng jobList đã lọc
    }
